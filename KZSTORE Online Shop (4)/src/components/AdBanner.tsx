...
export function AdBanner({ position, className = '' }: AdBannerProps) {
  ...
  const loadAds = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d8a4dffd/ads/active/${position}`,
        {
          headers: {
  ...
  const trackView = async (adId: string) => {
    try {
      await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d8a4dffd/ads/${adId}/view`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${publicAnonKey}`,
  ...
  const trackClick = async (adId: string) => {
    try {
      await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d8a4dffd/ads/${adId}/click`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${publicAnonKey}`,
        ...
        >
          ...
          {(currentAd.titulo || currentAd.descricao) && (
            <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
              {currentAd.titulo && (
                <h3 className="text-3xl font-bold mb-2">{currentAd.titulo}</h3>
              )}
              {currentAd.descricao && (
                <p className="text-lg text-white/90">{currentAd.descricao}</p>
              )}
            </div>
          )}
        </div>
