...
export function ProductReviews({ productId, productName, userEmail, userName, accessToken }: ProductReviewsProps) {
  ...
  const loadReviews = async () => {
    try {
      setLoading(true);
      // Buscar reviews diretamente da API REST do Supabase
      const response = await fetch(
        `https://${projectId}.supabase.co/rest/v1/reviews?product_id=eq.${productId}&is_approved=eq.true&order=created_at.desc`,
        {
          headers: {
  ...
  const handleSubmitReview = async () => {
    ...
    if (!accessToken || !userEmail || !userName) {
      console.error('❌ Dados de autenticação ausentes:', { accessToken: !!accessToken, userEmail, userName });
      toast.error('Por favor, faça login para avaliar este produto');
      return;
    }

    if (rating === 0) {
      console.error('❌ Avaliação não selecionada');
