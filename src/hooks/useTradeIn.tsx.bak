...
export function useTradeIn() {
  ...
  const fetchTradeIns = useCallback(async (): Promise<TradeIn[]> => {
    ...
    try {
      ...
      setTradeIns(data);
      console.log('✅ [useTradeIn] Trade-ins carregados:', data.length);
      return data;
    } catch (err) {
  ...
  const evaluateTradeIn = useCallback(async (
    ...
    setError(null);
    try {
      console.log('🔍 [useTradeIn] Avaliando trade-in:', id, evaluation);
      
      const response = await fetch(`/api/trade-in/${id}`, {
        method: 'PUT',
        headers: {
      ...
      const data = await response.json();
      console.log('✅ [useTradeIn] Trade-in avaliado:', data);
      ...
      console.error('❌ [useTradeIn] Erro ao avaliar:', err);
      setError(String(err));
      return false;
    } finally {
      setLoading(false);
