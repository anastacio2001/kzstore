...
export function AdminPanel({ onBack }: AdminPanelProps) {
  ...
  const handleSyncUsers = async () => {
    ...
    try {
      ...
      // Usar Edge Function para sincronizar usuários
      const response = await fetch(`${API_BASE}/customers/sync`, {
        method: 'POST',
        headers: getHeaders(),
      });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div className="border-b">
          <div className="flex gap-6 overflow-x-auto">
            ...
            >
              <Star className="inline-block mr-2 size-4" />
              Avaliações
            </button>
            <button
      ...
      {activeTab === 'reviews' && (
        <div>
          <h2 className="mb-6">Gestão de Avaliações</h2>
          <ReviewManagement accessToken={user?.access_token || ''} />
        </div>
      )}
