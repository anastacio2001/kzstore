...
export function HomePage({ 
  ...
  useEffect(() => {
    ...
    // Escutar por atualizações das configurações
    const handleSettingsUpdate = (event: CustomEvent) => {
      setHeroSettings(event.detail);
    };

    window.addEventListener('heroSettingsUpdated' as any, handleSettingsUpdate);

    return () => {
      window.removeEventListener('heroSettingsUpdated' as any, handleSettingsUpdate);
    };
  }, []);
  ...
  return (
    <div className="min-h-screen">
      ...
      <section 
        className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-red-900 to-gray-900"
        style={{
      ...
      {/* Available Coupons Section - Compact */}
      <section className="py-8 bg-gradient-to-r from-red-50/50 via-white to-yellow-50/50 border-y border-red-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AvailableCoupons showApplyButton={false} />
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {categories.map((category, index) => (
              <button
                ...
                className="group relative overflow-hidden bg-white rounded-xl sm:rounded-2xl p-4 sm:p-8 shadow-md hover:shadow-xl transition-all hover-lift"
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-xs sm:text-base text-gray-600 font-medium">{category.count} produtos</p>
                </div>

                <ArrowRight className="absolute bottom-3 right-3 sm:bottom-6 sm:right-6 size-4 sm:size-5 text-red-600 opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
            ))}
          </div>
        </div>
