...
export default function PreOrderForm({ product, trigger }: PreOrderFormProps) {
  ...
  // Usar percentual dinâmico ou 30% por padrão
  ...
  const uploadProof = async () => {
    ...
    try {
      ...
      console.log('📤 Uploading proof:', paymentProof.name);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      console.log('🔄 Iniciando upload do comprovativo...');
      ...
      const res = await fetch('/api/pre-orders', {
        ...
        body: JSON.stringify({
          ...
          notes: `${paymentMethod === 'multicaixa' ? 'Multicaixa Express' : 'Transferência Bancária'}\nTelefone: ${phone}`,
        }),
      });
      
      if (!res.ok) {
        const errorText = await res.text();
        console.error('❌ Erro ao criar pré-venda:', errorText);
        alert('Erro ao criar pré-venda. Tente novamente.');
        return;
      }
      
      const data = await res.json();
      console.log('✅ Pré-venda criada:', data.id);
      setPreOrderId(data.id);
      setStep(3);
    } catch (error) {
      console.error('❌ Erro geral:', error);
      alert('Erro ao processar pré-venda. Verifique sua conexão e tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Pague {depositPercentage}% agora • Restante na retirada</DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handlePaymentSubmit} className="space-y-4">
            ...
            <div><Label>Método</Label>
              <div className="grid grid-cols-2 gap-3 mt-2">
                ...
                <button type="button" onClick={() => setPaymentMethod('transfer')} className={`p-4 border-2 rounded ${paymentMethod === 'transfer' ? 'border-red-500 bg-red-50' : 'border-gray-200'}`}>
                  <Phone className="h-5 w-5 mb-2" /><p className="font-medium">Transferência</p>
                </button>
              </div>
            </div>
            ...
            {paymentMethod === 'transfer' && (
              <div className="p-4 bg-blue-50 border border-blue-200 rounded">
                <p className="text-sm mb-2 font-semibold text-blue-900">Dados para Transferência:</p>
