...
export function ProductDetailPage({ product, allProducts, onAddToCart, onBack, onProductClick, userEmail, userName, accessToken, onNavigateToProduct }: ProductDetailPageProps) {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        // Gerar slug legível a partir do nome do produto
        ...
        // URL limpa e legível: /produto/nome-do-produto-id
        const productUrl = `${window.location.origin}/produto/${slug}-${product.id}`;
        
        await navigator.share({
          title: product.nome,
          url: productUrl, // Apenas URL, sem text para evitar duplicação
        });
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 lg:py-12">
            ...
            <div className="relative aspect-square sm:aspect-square bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-gray-100 sm:border-2 group">
              ...
              <div className="absolute top-2 sm:top-4 right-2 sm:right-4 flex flex-col gap-1.5 sm:gap-2">
                ...
                <button
                  onClick={() => {
                    const slug = product.nome
                      ...
                      .trim();
                    const productUrl = `${window.location.origin}/produto/${slug}-${product.id}`;
                    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${product.nome} - ${productUrl}`)}`;
                    window.open(whatsappUrl, '_blank');
                  }}
                  className="size-9 sm:size-12 rounded-lg sm:rounded-xl bg-green-500 text-white shadow-lg hover:scale-110 transition-all flex items-center justify-center hover:bg-green-600"
                ...
                <button
                  ...
                  className="size-9 sm:size-12 rounded-lg sm:rounded-xl bg-blue-600 text-white shadow-lg hover:scale-110 transition-all flex items-center justify-center hover:bg-blue-700"
        ...
        <div className="mt-12 grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Ad Banner */}
            <AdBanner position="product-sidebar" onNavigateToProduct={onNavigateToProduct} />
          </div>
        </div>
